...
void print_error_message(sgx_status_t ret) {
  cout << "SGX error code: " << to_string(ret) << endl;
}
...
int initialize_enclave(sgx_enclave_id_t* eid, const string& launch_token_path, const string& enclave_name) {
  ...
  if (fp != NULL) {
    ...
    if (read_num != 0 && read_num != sizeof(sgx_launch_token_t)) {
      ...
      cout << "Warning: Invalid launch token read from " << token_path << "." << endl;
    }
    if (read_num != 0)
  ...
  if (write_num != sizeof(sgx_launch_token_t))
    cout << "Warning: Failed to save launch token to " << token_path << "." << endl;
  fclose(fp);
  return is_token_present ? 0 : 1;
}

bool is_ecall_successful(sgx_status_t sgx_status, const string& err_msg, int ret_value) {
  if (sgx_status != SGX_SUCCESS || ret_value < 0) {
    cout << err_msg << endl;
