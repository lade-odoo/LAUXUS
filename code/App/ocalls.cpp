...
int EMUL_API ocall_dump_in_dir(const char *dir, const lauxus_uuid_t *n_uuid, size_t size, const uint8_t *content) {
...
int EMUL_API ocall_dump_with_offset_in_dir(const char *dir, const lauxus_uuid_t *n_uuid, long offset, size_t size, const uint8_t *content) {
  string path(dir); path.append("/"); path.append(n_uuid->v);
  return dump_with_offset(path, offset, size, content);
}
int EMUL_API ocall_truncate_file_in_dir(const char *dir, const lauxus_uuid_t *n_uuid, long new_size) {
  string path(dir); path.append("/"); path.append(n_uuid->v);
  return truncate_file(path, new_size);
}


int EMUL_API ocall_load_file(const char *dir, const lauxus_uuid_t *n_uuid, long offset, size_t size, uint8_t *content) {
  string path(dir); path.append("/"); path.append(n_uuid->v);
  return load_with_offset(path, offset, size, content);
}


int EMUL_API ocall_file_size(const char *dir, const lauxus_uuid_t *n_uuid) {
  string path(dir); path.append("/"); path.append(n_uuid->v);
  return file_size(path);
}


int EMUL_API ocall_delete_from_dir(const char *dir, const lauxus_uuid_t *n_uuid) {
  string path(dir); path.append("/"); path.append(n_uuid->v);
