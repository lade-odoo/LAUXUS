#include "../../catch.hpp"
#include "../../../utils/node/filenode_content.hpp"
...
void _verify_read(FilenodeContent *content, long offset, size_t data_size, string expected) {
  char buffer[data_size];
  REQUIRE( content->read(offset, data_size, buffer) == data_size );
  REQUIRE( memcmp(buffer, expected.data(), data_size) == 0 );
}

void _verify_encrypt(FilenodeContent *content, long offset, size_t data_size, size_t expected_out, string expected) {
  size_t buffer_size = content->e_size(offset, data_size);
  char e_buffer[buffer_size], r_buffer[data_size];
  ...
  REQUIRE( memcmp(r_buffer, expected.data(), data_size) == 0 );
}
...
TEST_CASE( "2: Content can be read and written", "[multi-file:filenode_content]" ) {
  map<size_t, AES_CTR_context*> *keys = new map<size_t, AES_CTR_context*>();
  ...
  REQUIRE( content->write(0, 16, "This is a test !") == 16 );
  REQUIRE( content->size == 16 );
  REQUIRE( content->write(10, 23, "more complicated test !") == 23 );
  REQUIRE( content->size == 33 );

  REQUIRE( content->read(0, 0, NULL)  == 0 );
  REQUIRE( content->read(10, 0, NULL)  == 0 );
  _verify_read(content, 0, 33, "This is a more complicated test !");
  _verify_read(content, 10, 23, "more complicated test !");
  _verify_read(content, 0, 10, "This is a ");
...
TEST_CASE( "3: Content can be dumped to buffer and loaded", "[multi-file:filenode_content]" ) {
  ...
  _verify_encrypt(content, 0, 16, 0, "This is a test !");
  _verify_encrypt(content, 0, 11, 0, "This is a t");
  _verify_encrypt(content, 0, 10, 0, "This is a ");
  _verify_encrypt(content, 5, 10, 0, "is a test ");
  _verify_encrypt(content, 11, 3, 10, "est");
